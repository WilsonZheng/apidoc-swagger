...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	...
	if (verbDefinitionResult.topLevelSuccessRef) {
		...
		if(verbDefinitionResult.topLevelSuccessRefType){
...
function createPathParameters(verbs) {
	...
	if (verbs.parameter && verbs.parameter.fields["URL Parameter"]) {
		for (var i = 0; i < verbs.parameter.fields["URL Parameter"].length; i++) {
			...
			var inData = undefined;
			if(param.type.toLowerCase() === "date"){
				type = "string";
				format = "date";
			}else if(param.type.toLowerCase() === "datetime"){
				type = "string";
				format = "datetime";
			}else{
