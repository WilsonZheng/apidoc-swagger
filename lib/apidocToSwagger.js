...
// - Add feature of auto generating example for API success response (Not supporting array response)
...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName, examples) {
	...
	if (examples) {
		...
		try {
			// In Junifer, most the api doc json type has no double quotation for object property.
			...
			if (successExample[0]) {
				validJsonString = successExample[0].content
					.split("\n").map((sinleLine) => sinleLine.replace(/^\s*([a-zA-Z0-9-]+):/g, "\"$1\":")).join("\n");
	...
	for (var i = 0; i < fieldArray.length; i++) {
		...
		if (nestedName.propertyName) {
			...
			if (prop.type.toLowerCase() == "object") {
				...
				// TODO: It's not supporting array yet.
				if (!parameter.group.includes("Error")) {
					try {
						prop.example = exampleContent[nestedName.propertyName];
					} catch (e) {
						throw new Error("Cannot get successful response examples. Please check if the input file's Success-Response session contains the related data.\n" + e.message);
					}
				}
			}
			if (type) {
